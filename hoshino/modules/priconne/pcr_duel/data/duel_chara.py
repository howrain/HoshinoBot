...
class Roster:
    ...
    def update(self):
        ...
        importlib.reload(_dlc_data)
        self._roster.clear()
        pcr_chara = _pcr_data.CHARA_NAME
        dlc_chara = _dlc_data.DLC_CHARA_NAME
        charas = pcr_chara.copy()
        charas.update(dlc_chara)
        for idx, names in charas.items():
            for n in names:
...
class Chara:
    ...
    def name(self):
        ...
        charas = pcr_chara.copy()
        charas.update(dlc_chara)
        return charas[self.id][0] if self.id in charas else _pcr_data.CHARA_NAME[UNKNOWN][0]
    ...
    def icon(self):
        ...
        res = R.img(f'priconne/{unit_type}/icon_unit_{self.id}{star}1.png')
        if not res.exist:
            res = R.img(f'priconne/{unit_type}/icon_unit_{self.id}31.png')
        if not res.exist:
            res = R.img(f'priconne/{unit_type}/icon_unit_{self.id}11.png')
        if not res.exist and download_flag:   # FIXME: 不方便改成异步请求
